...
def add_to_vocab_book():
    ...
    try:
        ...
        # 检查单词是否已存在
        existing = session.query(VocabularyBook).filter_by(
            user_id=user_id, 
            word=word.lower()
        ).first()
...
def get_vocab_detail():
    ...
    session = get_db_session()
    try:
        vocab = session.query(VocabularyBook).filter_by(
            user_id=user_id,
            vocab_id=vocab_id
        ).first()
...
def batch_delete_vocab():
    ...
    try:
        # 批量删除
        deleted = session.query(VocabularyBook).filter(
...
def get_vocab_stats():
    ...
    try:
        ...
        mastered = session.query(VocabularyBook).filter_by(user_id=user_id, status=2).count()
        learning = session.query(VocabularyBook).filter_by(user_id=user_id, status=1).count()
        new_count = session.query(VocabularyBook).filter_by(user_id=user_id, status=0).count()
