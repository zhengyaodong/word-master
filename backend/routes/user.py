...
def get_user_stats():
    ...
    try:
        ...
        mastered_count = session.query(VocabularyBook).filter_by(user_id=user_id, status=2).count()
        learning_count = session.query(VocabularyBook).filter_by(user_id=user_id, status=1).count()
